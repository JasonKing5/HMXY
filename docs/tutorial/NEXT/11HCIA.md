...
准备变量数据`JJname`接收被拉起方返回的结果数据。当该变量数据被结果数据覆盖时，渲染UI更新。所以需要状态装饰器。
...
> 为了能够支持被其他应用访问，目标应用需要在`module.json5`配置文件中声明`URLskill`。其中，`uris`字段的 `scheme` 的取值支持自定义，可以定义为任意不包含特殊字符、非`ohos` 开头的字符串。
>
