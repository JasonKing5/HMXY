...
![createCard](screenshots/07服务卡片/createCard.png)

在已有的应用工程中，可以通过右键新建`ArkTS卡片`，具体的操作方式如下。

右键新建卡片。

![alt text](screenshots/07服务卡片/createCard1.png)
...
2 根据实际业务场景，选择一个卡片模板。
 ...
 ![choose](screenshots/07服务卡片/choose.png)

 
3 在选择卡片的开发语言类型（`Language`）时，选择`ArkTS`选项，然后单击“`Finish`”，即可完成`ArkTS卡片`创建。

![setProject](screenshots/07服务卡片/setProject.png)
...
![CardFiles](screenshots/07服务卡片/CardFiles.png)
...
![showCard](screenshots/07服务卡片/showCard.png)
...
![setCardData](screenshots/07服务卡片/setCardData.png)
...
提供方主动刷新卡片流程示意：
![alt update](screenshots/07服务卡片/update.png)
...
### 动态卡片事件能力说明
![alt DynamicCard](screenshots/07服务卡片/DynamicCard.png)
...
![alt entryType](screenshots/07服务卡片/entryType.png)

1. 创建应用页面文件方便通过卡片入口拉起，模拟收集管家拉起不同页面

![alt routerPage](screenshots/07服务卡片/routerPage.png)
...
----
### call类型
![alt addNumber](screenshots/07服务卡片/addNumber.png)
...
![alt messageType](screenshots/07服务卡片/messageType.png)
...
### 定点刷新
![alt setTimeUpdate](screenshots/07服务卡片/setTimeUpdate.png)
> 指定时间点更新数据，例如上图所示，`09:37`分定时更新数据，由数字`1`更新为`666`
---
1. 准备卡片页面
![alt prepareCardPage](screenshots/07服务卡片/prepareCardPage.png)
>卡片页面布局效果
```
let localStorage = new LocalStorage()
